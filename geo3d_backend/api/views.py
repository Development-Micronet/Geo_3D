...
def users_view(request):
    ...
    if request.method == "GET":
        ...
        if role in ("admin", "superadmin") and admin.role != "superadmin" and not admin.is_superuser:
            return JsonResponse({"detail": "Only superadmin can create admin accounts"}, status=403)
...
def user_detail_view(request, user_id):
    ...
    if (user.role in ("admin", "superadmin") or user.is_staff or user.is_superuser) and not admin.is_superuser and admin.role != "superadmin":
        return JsonResponse({"detail": "Only superadmin can delete admin accounts"}, status=403)
...
def serve_layer_resource(request, package_id: str, resource_path: str = ""):
    ...
    for cand in candidates:
        ...
        if resolved is None:
            return JsonResponse({"detail": "Invalid resource path"}, status=400)
        if resolved.exists():
            target = resolved
            break
